...
import { Text, View, Modal, TouchableOpacity, Image, FlatList, Linking, Dimensions } from 'react-native';
...
const CustomModal = ({
    ...
    const renderQuestionitem = ({ item, index }) => {
        ...
        // let _remarkUpdated = remark.replace(/(<|&lt;)style\s*\/*(>|&gt;)/g, "");
        // _remarkUpdated = _remarkUpdated.replace(/(<|&lt;)style\s*\/*(>|&gt;)/g, "")
        // _remarkUpdated = _remarkUpdated.replace(/<[^/>][^>]*><\/[^>]+>/g, "")
        return (
            <View style={{ flexDirection: 'row', marginVertical: _scaleText(5).fontSize, marginHorizontal: _scaleText(10).fontSize, borderWidth: 0.5, padding: _scaleText(5).fontSize, borderRadius: _scaleText(5).fontSize }}>
                ...
                <View style={{ borderWidth: 0, marginLeft: _scaleText(5).fontSize }}>
                    <View>
                        ...
                        <HTMLView style={{ width: '95%' }} stylesheet={styles} value={_quest} />
                        {/* <View style={{ width: '97%' }}>
